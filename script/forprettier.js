function toggleShelf() {
    ...
    // Preprocessing gambar: ubah ukuran & normalisasi
    function preprocessImage(imgElement) {
      return tf.tidy(() => {
        let tensor = tf.browser.fromPixels(imgElement)
          .resizeNearestNeighbor([380, 380]) // ubah sesuai input model
          .toFloat()
          .div(tf.scalar(255.0)) // normalisasi 0-1
    ...
    async function predictImage(imgElement) {
      ...
      document.getElementById('prediction').innerText = 
        'Hasil Prediksi: ' + Array.from(predictions).map(x => x.toFixed(3));
    }
